...
class ScrollPages:UIView,UIScrollViewDelegate{
    ...
    func setupUI(){
        ...
        imageView0_3.frame = CGRect(x:0, y: 0, width: pageWidth, height: self.bounds.height)
        imageView1.frame = CGRect(x: pageWidth, y: 0, width: pageWidth, height: self.bounds.height)
        imageView2.frame = CGRect(x: pageWidth*2, y: 0, width: pageWidth, height: self.bounds.height)
        imageView3.frame = CGRect(x: pageWidth*3, y: 0, width: pageWidth, height: self.bounds.height)
        imageView4_1.frame = CGRect(x: pageWidth*4, y: 0, width: pageWidth, height: self.bounds.height)
        imageView0_3.backgroundColor = .green
         imageView1.backgroundColor = .blue
         imageView2.backgroundColor = .yellow
         imageView3.backgroundColor = .green
         imageView4_1.backgroundColor = .blue
         self.addSubview(scrView)
        scrView.addSubview(imageView0_3)
         scrView.addSubview(imageView1)
         scrView.addSubview(imageView2)
         scrView.addSubview(imageView3)
         scrView.addSubview(imageView4_1)
         
         scrView.isPagingEnabled = true //翻页属性
         scrView.contentOffset = CGPoint(x: pageWidth, y: 0) //图大w显示位置
